...
Desse aspecto surge a denominação de \textit{partially greedy cyclic}. A reordenação é feita uma única vez antes da execução, diferentemente do método \textit{greedy} CD que seleciona a melhor coordenada para cada atualização. Outras alternativas de ordem de atualização podem ser empregadas, como randômica ou cíclica convencional, embora experimentos numéricos demonstraram a superioridade da estratégia em questão \cite{fastselect}. 
...
\label{alg:cpsi}
\Input{$\hat{x}^0 \in \R^p$, $k \in \mathbb{N}^+$ and an inner algorithm}\\

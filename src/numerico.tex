...
\subsection{\textit{Setup} experimental}\label{sec:setup}
\subsubsection{Geração de dados}
Assim como em \cite{fastselect}, utilizamos conjuntos de dados sintéticos para uma ampla variedade de tamanhos e configurações de problemas. A matriz de dados é gerada com distribuição gaussiana multivariada $A_{n \times p} \sim \text{MVN}(0, \Sigma)$, e então suas colunas são normalizadas tal que $\|A_i\|_2=1\ \forall i$ para simplificar as atualizações dos algoritmos. Como verdade base, é utilizado um vetor de coeficientes esparsos $x^\dagger\in\R^p$ com $k^\dagger$ entradas não nulas, cujos valores e suporte são definidos conforme cada experimento. É escolhido também um valor aproximado para o SNR, definido como 
...
Após experimentos iniciais de ajuste de hiperparâmetros, foram escolhidos $m=15,\ \delta=0.01$ e $\tau=0.25$ para o NSPG. As salvaguardas $\gamma_{min}$ e $\gamma_{max}$ são usualmente tomadas como valores arbitrariamente pequenos e grandes, respectivamente. Os tamanhos de passo espectral iniciais, $\gamma^{BBR}_{0,+}$ e $\gamma^{BBR2}_{0,+}$, foram tomados via \eqref{BBR} e \eqref{BBR2} com a convenção de $x^{-1}=x^0-10^{-5}\nabla f(x^0)$. Já a métrica diagonal espectral inicial, $U_{0,+}^{DBBR}$, foi definida com esses tamanhos de passo e a convenção de $U_{-1,+}^{DBBR}$ nula. Para o VMNSPG, $\mu=10^{-3}$ apresentou os melhores resultados dentre os valores testados. Quanto ao PGCCD, somente foram ordenadas um quarto das coordenadas.



\subsection{Comparação entre PGCCD e NSPG}
Consideramos a eficiência dos algoritmos propostos para o problema \eqref{R} com $\lambda_q=0$ (problema $\ell_0$ puro). Foi utilizado um conjunto de dados com correlação exponencial, $\rho=0.5,\ n=500,\ p=2000,\ k^\dagger=100,\ \text{SNR}=10$. Foram geradas 50 instâncias (inicializações) aleatórias do problema, em que para cada uma o suporte de $x^\dagger$ foi escolhido uniformemente de $[p]$ e seus valores tomados com distribuição uniforme em $(0,1)$. Os algoritmos foram inicializados a partir de vetores com esse mesmo suporte e valores independentes na mesma distribuição uniforme. A medida de recuperação do suporte é definida como 
\[\frac{| Supp(x) \cap Supp(x^\dagger) |}{\max \{ |Supp(x)|, k^\dagger \}},\]
que quantifica a fração de variáveis verdadeiramente ativas recuperadas, normalizada pelo máximo entre a dimensão do suporte estimado e o verdadeiro $k^\dagger$. Essa medida penaliza tanto falsos positivos quanto falsos negativos e valores próximos de 1 indicam recuperação alta. O critério de parada utilizada em todos os algoritmos foi uma mudança relativa no objetivo menor que $10^{-7}$.

A \autoref{pK} contém o \textit{box plot} do número de iterações, em que uma passagem pelas $p$ coordenadas corresponde a uma iteração do PGCCD. Essa métrica não reflete precisamente a velocidade de cada algoritmo uma vez que não considera, por exemplo, o número de sub iterações na busca linear do NSPG e a reordenação efetuada no PGCCD. Mesmo assim, é possível ter uma ideia de que a convergência do PGCCD é significativamente mais rápida que a dos métodos proximais não monótonos apresentados. O VMNSPG demonstra convergência mais rápida entre os métodos NSPG, provavelmente devido à sua incorporação de mais informação de segunda ordem. Seu número de iterações, com mediana em torno de 30, fica o mais próximo do PGCCD, com mediana em torno de 20. O passo espectral híbrido do NSPGH parece acelerar levemente a convergência em comparação com o passo primal usado no NSPG, e ambos performam cerca de duas a três vezes pior que o PGCCD.
...
\begin{figure}[H]
  ...
  \caption{Resultados para 50 inicializações. Correlação exponencial, $\rho=0.5,\ n=500,\ p=2000,\ k^\dagger=100,\ \text{SNR}=10$.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Resultados para 50 inicializações. Correlação exponencial, $\rho=0.5,\ n=500,\ p=2000,\ k^\dagger=100,\ \text{SNR}=300$.}\label{p3}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Resultados para 50 inicializações. Correlação constante, $\rho=0.5,\ n=500,\ p=2000,\ k^\dagger=100,\ \text{SNR}=300$.}\label{p4}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Resultados para 50 inicializações. Correlação exponencial, $\rho=0.9,\ n=250,\ p=1000,\ k^\dagger=25,\ \text{SNR}=300$.}\label{p5}
\end{figure}
...
Observa-se que métodos baseados em NSPG beneficiam-se significativamente de caminhos de regularização bem definidos. Para o NSPG, observa-se que a estratégia inversa é altamente competitiva e robusta na maioria dos cenários. Já para o PGCCD, a estratégia adaptativa (\textit{Smart Adaptive}) mostrou-se a mais estável e rápida para garantir boa recuperação em todos os cenários. A estratégia adaptativa também mostrou-se ideal para a combinação NSPG+PGCCD, equilibrando eficiência e evasão de mínimos locais inadequados. Assim, os resultados reportados nas seções seguintes utilizam CV Inversa para NSPG, e CV Adaptativa para NSPG+PGCCD e PGCCD.
...
\begin{figure}[H]
  ...
  \caption{Comparação de estratégias de validação cruzada. Correlação exponencial, $\rho=0.5,\ p=2000,\ k^\dagger=100,\ \text{SNR}=10$.}
  \label{fig:cv_comp}
\end{figure}


\subsection{Comparação com L0Learn}
Para contextualizar o desempenho do PGCCD em relação ao estado da arte, realizamos uma comparação com o pacote \texttt{L0Learn} \cite{l0learn} (versão de desenvolvimento do repositório \texttt{git} \url{https://github.com/fbkns/L0Learn}). O \texttt{L0Learn} é altamente otimizado e implementa algoritmos de descida coordenada com estratégias sofisticadas de \textit{active set}. O setup de dados segue o padrão descrito na \autoref{sec:setup}, com a verdade base $x^\dagger$ possuindo $k^\dagger$ entradas igualmente espaçadas e com valor $1$. Para cada configuração de tamanho de amostra $n$, os resultados reportados representam a média sobre 10 instâncias (trials) independentes.

Nesta comparação, distinguimos três variantes:
(i) \texttt{L0Learn+CPSI(1) val}: utiliza o algoritmo nativo \texttt{CPSI(1)} da versão Git;
(ii) \texttt{L0Learn val (CD only)}: utiliza o algoritmo nativo \texttt{CD};
(iii) PGCCD+CPSI(1): utiliza nossa própria implementação de descida coordenada (PGCCD) combinada com a busca combinatorial CPSI(1) e estratégia de validação cruzada adaptativa.
...
A \autoref{fig:l0learn_comp} apresenta os resultados para o cenário de correlação exponencial com $\rho=0.5$ e $p=2000$. Neste cenário, todos os métodos baseados em NSPG atingem recuperação quase perfeita do suporte a partir de $n \approx 800$. A combinação NSPG+PGCCD+CPSI(1) atinge similaridade 1.0 ligeiramente antes dos demais. Para mitigar o custo computacional do PGCCD em instâncias de alta dimensionalidade, introduzimos um critério de parada heurístico (\textit{skip}): a descida coordenada só é executada se o número de variáveis ativas na iteração atual for inferior a $p/2$. Essa estratégia concentra o esforço computacional apenas nas fases finais de ajuste fino da solução, permitindo que o método se mantenha altamente competitivo em tempo de execução enquanto preserva a qualidade superior na recuperação do suporte. Quanto ao erro $L_\infty$ nos parâmetros, os métodos NSPG+CPSI(1) e NSPG+PGCCD+CPSI(1) apresentam os menores valores em todo o intervalo de $n$, indicando que a melhor recuperação do suporte traduz-se diretamente em melhor estimação dos parâmetros. O método \texttt{L0Learn val (CD only)} é consistentemente o pior tanto em similaridade quanto em $L_\infty$.

\begin{figure}[H]
  \centering
  \subbottom[Similaridade do Suporte.]{%
    \label{l0_sup}%
    \includegraphics[width=0.45\linewidth]{\mixedPath exp-0.5-2000/pnSim-VariableN_L0_exp-0.5-2000-10.0-100-10-200_200_2000_false.png}}
  \subbottom[Tempo de execução (s).]{%
    \label{l0_time}%
    \includegraphics[width=0.45\linewidth]{\mixedPath exp-0.5-2000/pnTime-VariableN_L0_exp-0.5-2000-10.0-100-10-200_200_2000_false.png}}\\
  \subbottom[Erro Máximo nos Parâmetros ($L_\infty$).]{%
    \label{l0_inf}%
    \includegraphics[width=0.45\linewidth]{\mixedPath exp-0.5-2000/pnInf-VariableN_L0_exp-0.5-2000-10.0-100-10-200_200_2000_false.png}}
  \caption{Comparação com o L0Learn. Correlação exponencial, $\rho=0.5,\ p=2000,\ k^\dagger=100,\ \text{SNR}=10$.}
  \label{fig:l0learn_comp}
\end{figure}

A vantagem do PGCCD torna-se mais pronunciada nos cenários de correlação constante, como ilustrado na \autoref{fig:l0learn_const}. Neste cenário com $\rho=0.9$ e $p=1000$, a alta correlação dificulta a seleção de variáveis para todos os métodos, porém tanto o NSPG+CPSI(1) quanto o NSPG+PGCCD+CPSI(1) apresentam superioridade consistente. O NSPG+PGCCD+CPSI(1) atinge similaridade $\sim$0.95 já em $n \approx 250$, enquanto as variantes nativas do \texttt{L0Learn} necessitam de $n \geq 400$ para atingir desempenho comparável. O \texttt{L0Learn val (CD only)}, em particular, apresenta degradação significativa neste cenário de alta correlação. Em termos de erro $L_\infty$, os métodos NSPG mantêm vantagem em toda a faixa de $n$, convergindo para $\sim$0.1 a partir de $n \approx 800$.
...
\begin{figure}[H]
  ...
  \caption{Comparação com o L0Learn. Correlação constante, $\rho=0.9,\ p=1000,\ k^\dagger=20,\ \text{SNR}=5$.}
  \label{fig:l0learn_const}
\end{figure}

Essa diferença substancial nos cenários de correlação constante ocorre porque a descida coordenada do PGCCD é particularmente eficaz quando as correlações entre variáveis são uniformes, permitindo uma exploração mais eficiente do espaço de soluções. O mesmo padrão é observado na \autoref{fig:l0learn_anexo_1} ($\rho=0.5,\ p=2000$), onde o NSPG+PGCCD+CPSI(1) alcança similaridade próxima a 1.0 já em $n \approx 400$ com custo temporal moderado ($\sim$22s para $n=2000$). No cenário de correlação exponencial com $\rho=0.9$ (\autoref{fig:l0learn_anexo_3}), as diferenças entre os métodos são menores e a convergência é mais uniforme, sugerindo que a estrutura exponencial decrescente das correlações facilita a separação das variáveis relevantes.

Em síntese, os resultados indicam que: (i) a combinação NSPG+PGCCD oferece a recuperação de suporte mais robusta, especialmente em cenários de correlação constante, ao custo de maior tempo de execução; (ii) o NSPG isolado é altamente competitivo em cenários de correlação exponencial, onde alcança excelente recuperação com custo computacional reduzido; (iii) o método PGCCD+CPSI(1), que aplica nossa implementação de descida coordenada e procedimento de validação cruzada, apresenta desempenho significativamente superior às variantes nativas do \texttt{L0Learn}, evidenciando a importância da estratégia de busca; e (iv) o erro $L_\infty$ confirma consistentemente que a qualidade da recuperação do suporte se traduz em melhor estimação dos parâmetros verdadeiros.

\subsection{Experimentos com Estrutura Tridiagonal em Blocos}

Para explorar o comportamento dos algoritmos sob uma estrutura de covariância mais restrita e esparsa, realizamos experimentos nos quais a matriz de dados $A$ foi modificada para possuir uma estrutura quase tridiagonal em blocos. Especificamente, após a geração padrão $A \sim \text{MVN}(0, \Sigma)$, aplicamos uma máscara que zera as entradas fora de uma faixa diagonal estreita (aproximadamente 3 linhas não nulas por parâmetro de forma balanceada, com o caso $n=p$ tornando-se uma matriz tridiagonal). Essa modificação isola os blocos de variáveis, quebrando a dependência de características globais enquanto preserva a estrutura de covariância local ditada por $\Sigma$ no interior de cada bloco.

A eliminação de conexões globais densas tende a simplificar a paisagem de otimização, auxiliando métodos de segunda ordem estruturados ou buscas combinatoriais. No cenário de correlação constante extrema ($\rho=0.9$, \autoref{fig:tri_const}), observamos que a máscara tridiagonal fornece um ganho substancial em recuperação do suporte se comparado ao seu análogo denso (\autoref{fig:l0learn_const}). Sem a intrincada teia de dependências globais, os algoritmos, especialmente aqueles com estratégias ativas e combinatoriais como o NSPG+PGCCD, isolam a alta correlação aos pequenos blocos e conseguem separar o suporte verdadeiro com muito mais facilidade. 

Por outro lado, o cenário de correlação exponencial (\autoref{fig:tri_exp}) exibe um fenômeno contraintuitivo: sua recuperação do suporte sofre uma degradação perceptível se comparada à sua contraparte densa (\autoref{fig:l0learn_anexo_3}). Uma explicação provável para essa inversão de desempenho está ligada à forma como a máscara afeta o poder estatístico de dados com covariância decrescente. Na matriz exponencial original, as correlações decaem progressivamente ($0.9, 0.81, 0.729 \dots$), fornecendo informações residuais importantes através de observações não locais. Ao aplicar a máscara, essas observações complementares são zeradas. Como resultado, o algoritmo é forçado a diferenciar as variáveis dependendo quase exclusivamente da informação no pequeno bloco local que, por sua vez, mantém sua correlação altíssima (0.9). O cenário constante, cuja matriz original não oferecia decaimento que ajudasse a diferenciar as variáveis globalmente, se beneficia unicamente da redução drástica da conectividade densa.

A sensibilidade dos algoritmos a essas diferentes estruturas de correlação localizada evidencia que a topologia do problema afeta drasticamente a dificuldade da otimização. Esses resultados motivam futuras investigações na substituição da métrica variável diagonal (que se mostrou pouco efetiva no VMNSPG) por operadores \textit{proximais} com matrizes de segunda ordem estruturadas (ex: tridiagonais). Tais métodos podem oferecer ganhos de convergência e precisão significativos ao introduzir mais informação de segunda ordem focada nessas dependências locais fortes, mantendo um custo computacional consideravelmente inferior ao de abordagens Newton densas. Gráficos adicionais para cenários de SNR maior e $\rho=0.5$ estão dispostos no \autoref{chap:anexos_tri}.

\begin{figure}[H]
  \centering
  \subbottom[Similaridade do Suporte.]{%
    \label{tri_const_sup}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/const-0.9-1000/pnSim-NewTerminal_mixed_v2_tri_const_0.9_p1000.png}}
  \subbottom[Tempo de execução (s).]{%
    \label{tri_const_time}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/const-0.9-1000/pnTime-NewTerminal_mixed_v2_tri_const_0.9_p1000.png}}\\
  \subbottom[Erro Máximo nos Parâmetros ($L_\infty$).]{%
    \label{tri_const_inf}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/const-0.9-1000/pnInf-NewTerminal_mixed_v2_tri_const_0.9_p1000.png}}
  \caption{Experimento tridiagonal. Correlação constante, $\rho=0.9,\ p=1000,\ k^\dagger=20,\ \text{SNR}=5$.}
  \label{fig:tri_const}
\end{figure}
...
\begin{figure}[H]
  ...
  \subbottom[Similaridade do Suporte.]{%
    \label{tri_exp_sup}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/exp-0.9-1000/pnSim-NewTerminal_mixed_v2_tri_exp_0.9_p1000.png}}
  \subbottom[Tempo de execução (s).]{%
    \label{tri_exp_time}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/exp-0.9-1000/pnTime-NewTerminal_mixed_v2_tri_exp_0.9_p1000.png}}\\
  \subbottom[Erro Máximo nos Parâmetros ($L_\infty$).]{%
    \label{tri_exp_inf}%
    \includegraphics[width=0.45\linewidth]{images/Mixed_v2/Tridiagonal/exp-0.9-1000/pnInf-NewTerminal_mixed_v2_tri_exp_0.9_p1000.png}}
  \caption{Experimento tridiagonal. Correlação exponencial, $\rho=0.9,\ p=1000,\ k^\dagger=20,\ \text{SNR}=5$.}
  \label{fig:tri_exp}
\end{figure}

\subsection{Escolha de Limiares para Penalização Mista $L_0 + L_1 + L_2$}

No contexto da penalização mista, os limiares de redefinição para o parâmetro $\lambda_0$ são adaptados para incorporar os efeitos de $L_1$ ($\lambda_1$) e $L_2$ ($\lambda_2$). No passo proximal com passo $\gamma$, de \eqref{eq:proxl0l1l2}, a condição para uma coordenada entrar no suporte a partir do ponto $x$ exige
\[
\max(0, |[x - \gamma \nabla f(x)]_l| - \gamma \lambda_1) \ge \sqrt{2\lambda_0 \gamma (1 + 2\lambda_2 \gamma)}.
\]

Se partirmos de $x = 0$ (onde $\nabla f(0) = -X'y$), o limiar acima do qual a estimativa é garantida como nula no primeiro passo do gradiente proximal é obtido resolvendo a inequação para $\lambda_0$:
\[
\lambda_0 \le \frac{\gamma \left[\max\left(0, |\nabla f(0)|_l - \lambda_1\right)\right]^2}{2 (1 + 2\lambda_2 \gamma)}.
\]

Dessa forma, os limiares inicializadores de $\lambda_0$ para as validações padrão e inversa tornam-se, respectivamente:
\[
\lambda_0 = 1.01 \cdot \gamma_{0,0} \cdot \frac{\left[\max\left(0, \max_l |\nabla f(x^0)_l| - \lambda_1\right)\right]^2}{2 (1 + 2\lambda_2 \gamma_{0,0})},
\]
e
\[
\lambda_0 = 1.01^{-1} \cdot \gamma_{0,0} \cdot \frac{\left[\max\left(0, \min_l |\nabla f(x^0)_l| - \lambda_1\right)\right]^2}{2 (1 + 2\lambda_2 \gamma_{0,0})}.
\]

\subsubsection{Fórmulas Gerais de Atualização}

De forma análoga, os passos de redefinição ao longo dos caminhos de regularização da validação cruzada padrão (decrescente) e inversa (crescente) na iteração $j$ passam a ser expressos como:
\[
\lambda_{j+1} = 0.9 \cdot \min\left(\lambda_j, \gamma_{j,0} \cdot \frac{\left[\max\left(0, \max_{l \in S^c} |\nabla f(x^{j,0})_l| - \lambda_1\right)\right]^2}{2 (1 + 2\lambda_2 \gamma_{j,0})}\right),
\]
e
\[
\lambda_{j+1} = 0.9^{-1} \cdot \max\left(\lambda_j, \frac{\left[\max\left(0, \min_{l \in S} |x^{j,0}_l - \gamma_{j,0} \nabla f(x^{j,0})_l| - \gamma_{j,0} \lambda_1\right)\right]^2}{2\gamma_{j,0} (1 + 2\lambda_2 \gamma_{j,0})}\right),
\]
onde $S$ denota o suporte de $x^{j,0}$ e $S^c$ o seu complemento.

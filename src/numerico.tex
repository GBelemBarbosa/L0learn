...
(iii) \textbf{\texttt{L0Learn+CPSI(1) val path}}: utiliza o motor \texttt{CD} do \texttt{L0Learn} como base em um \textit{wrapper} \texttt{Julia} que permite chamadas para um par específico $(\lambda, x)$, aplicando nossa própria estratégia de validação cruzada padrão.
